...
// EMailAddress Queries
	...
	// Added by Himani
	public static void updateEmailAddressPhone(String inEMailAddress, String inWPhone, String inMPhone)
			throws Exception {
		Connection dbConn = null;
		PreparedStatement pStmt = null;
		int rowsUpdated = 0;

		System.out.println("Inside DBAccess : " + inEMailAddress + "  " + inWPhone + "  " + inMPhone);
		try {
			dbConn = getConnection();
			pStmt = dbConn
					.prepareStatement("UPDATE emailaddress SET workphoneno = ?, mobileno = ? WHERE emailaddress = ?");

			pStmt.setString(1, inWPhone);
			pStmt.setString(2, inMPhone);
			pStmt.setString(3, inEMailAddress);
			// pStmt.setString(2, inEMailAddress.getwPhone());
			// pStmt.setString(3, inEMailAddress.getmPhone());

			rowsUpdated = pStmt.executeUpdate();
			System.out.println("Rows Updated :" + rowsUpdated);

			if (rowsUpdated != 1) {
				throw new Exception("Error in updating phone numbers");
	...
	public static ArrayList getAllEMailAdressEntriesbyGroup(String inEmailAddress) throws Exception {
		...
		// Start Add by Himani
		System.out.println("In DBAccess : " + inEmailAddress);// Added by Himani

		try {
			dbConn = getConnection();
			// pStmt = dbConn.prepareStatement("SELECT * FROM emailaddress WHERE groupid = ?");
			pStmt = dbConn.prepareStatement(
					"SELECT * FROM emailaddress WHERE groupid = (SELECT groupid FROM emailaddress WHERE emailaddress = ?)");// Modified
																															// by
																															// Himani
			pStmt.setString(1, inEmailAddress);
			rs = pStmt.executeQuery();

			System.out.println("Checking Result Set : " + rs);// Added by Himani
			while (rs.next()) {
				eMailAddress = new EMailAddressVOO(rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4),
						rs.getString(5), rs.getString(6), rs.getString(7), rs.getString(8));
				eMailList.add(eMailAddress);
